...
class _HomeState extends State<Home> {
  ...
  void loadTodos() async {
    ...
    if (loadedTodos.isEmpty) {
      ...
      todoService.saveTodos(loadedTodos);
    }
    setState(() {
      allTodos = loadedTodos; // Store the complete list
      foundtodo = List.from(allTodos); // Display all todos initially
    });
  }
  ...
  void runfilter(String words) {
    ...
    if (words.isEmpty || words == "") {
      results = allTodos; // Filter based on the currently displayed list
    } else {
      results = allTodos
          .where((item) =>
  ...
  List<Todo> sortTodos(List<Todo> foundtodos) {
    List<Todo> done = [];
    List<Todo> undone = [];
    Todo todo;
    for (todo in foundtodos) {
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      ...
                      for (Todo item in sortTodos(foundtodo))
                        Todoitem(
                          todo: item,
